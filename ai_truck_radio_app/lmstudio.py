...
class LMStudioClient:
    ...
    def generate_host_line(self, previous_track: Optional[Track], next_track: Optional[Track], ctx: Dict[str, Any]) -> str:
        ...
        if force_duo_retry and ctx.get("two_hosts") and len(hosts) >= 2:
            ...
            clock_rule = (
                f"Точное локальное время компьютера сейчас: {ctx.get('time_text') or _current_time_text()}. Сейчас по времени суток: {daypart_now}. "
                ...
                "Если называешь время словами, сверяйся с этим значением. При сомнении просто скажи точное время цифрами."
            )
            creative_fact_rule = (
