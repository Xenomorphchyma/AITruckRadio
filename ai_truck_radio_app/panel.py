...
def render_panel(engine: Any, cfg: Dict[str, Any], snap: Dict[str, Any], default_config: Dict[str, Any], app_name: str, app_version: str) -> str:
    ...
    music_settings = "".join([
        ...
        checkbox("track_profiles_web_lookup_enabled", "Искать сведения о музыке в интернете", "Кнопка 'Сгенерировать описания' будет использовать MusicBrainz/Wikipedia, а не только название файла."),
        ...
        checkbox("track_profiles_enrich_only_if_no_sources", "Не трогать профили, где уже есть источники", "Если у трека уже есть sources/_web_meta, он не будет повторно обрабатываться только из-за web_fact='нет надёжного факта'."),
        ...
        input_text("track_profiles_web_lookup_provider", "Провайдеры", "Сейчас поддерживаются MusicBrainz/Wikipedia через встроенный скрипт."),
...
<style>
:root {{ --bg:#0b1020; --panel:#111a2f; --panel2:#151f38; --text:#eef3ff; --muted:#9ba9c7; --line:#293652; --accent:#7cc7ff; --good:#62d78d; --bad:#ff7575; --warn:#ffd166; --shadow:0 14px 40px rgba(0,0,0,.32); }}
* {{ box-sizing:border-box; }}
html, body {{ height:100%; overflow:hidden; }}
body {{ margin:0; font-family:Segoe UI,Arial,sans-serif; background:radial-gradient(circle at top left,#20345f 0,#0b1020 42%,#070a12 100%); color:var(--text); }}
button,input,select,textarea {{ font:inherit; }}
button {{ border:0; border-radius:11px; padding:9px 11px; color:#07101f; background:var(--accent); font-weight:800; cursor:pointer; transition:.12s filter,.12s transform; min-width:0; }}
...
.layout {{ display:grid; grid-template-columns:260px minmax(0,1fr); gap:14px; padding:14px; max-width:1540px; height:calc(100vh - 58px); margin:0 auto; overflow:hidden; }} @media(max-width:980px){{ html,body{{overflow:auto;height:auto;}} .layout{{grid-template-columns:1fr;height:auto;overflow:visible;}} }}
main {{ min-width:0; overflow:hidden; display:flex; flex-direction:column; }}
.card {{ background:linear-gradient(180deg,rgba(21,31,56,.94),rgba(15,23,42,.94)); border:1px solid var(--line); box-shadow:var(--shadow); border-radius:16px; padding:13px; min-width:0; }}
.side {{ min-width:0; overflow:hidden; display:grid; grid-template-rows:auto auto auto; gap:10px; align-content:start; }} .side .card {{ margin:0; }}
.now {{ font-size:15px; font-weight:900; margin:2px 0 7px; overflow-wrap:anywhere; line-height:1.18; }} .muted {{ color:var(--muted); }} .mini {{ font-size:11px; color:var(--muted); line-height:1.25; }}
.side-hero {{ display:grid; gap:9px; }} .side-title {{ display:flex; justify-content:space-between; gap:8px; align-items:center; }} .status-pills {{ display:grid; grid-template-columns:1fr 1fr; gap:6px; font-size:12px; }} .pill {{ background:rgba(0,0,0,.18); border:1px solid rgba(255,255,255,.05); border-radius:10px; padding:7px; min-width:0; overflow-wrap:anywhere; }} .pill b {{ color:#fff; }} .pill .ok {{ color:var(--good); }}
.mode-card {{ display:grid; gap:8px; padding:10px; border:1px solid rgba(124,199,255,.22); border-radius:14px; background:rgba(124,199,255,.06); }} .mode-actions {{ display:grid; grid-template-columns:1fr 1fr; gap:7px; }} .mode-actions button {{ min-height:44px; white-space:normal; line-height:1.15; }}
.controls {{ display:grid; gap:8px; }} .controls .pair {{ display:grid; grid-template-columns:1fr 1fr; gap:7px; }} .controls button {{ width:100%; white-space:normal; line-height:1.15; }}
.side-check {{ display:flex; align-items:center; gap:8px; padding:9px 10px; border:1px solid rgba(255,255,255,.06); border-radius:12px; background:rgba(0,0,0,.16); font-size:12px; }} .side-check input {{ width:auto; flex:0 0 auto; }}
.player-card {{ display:grid; gap:8px; }} .player-buttons {{ display:grid; grid-template-columns:1fr 1fr; gap:7px; }} audio {{ width:100%; max-width:100%; height:34px; display:block; }}
.tabs {{ display:flex; flex-wrap:nowrap; gap:8px; margin-bottom:10px; overflow-x:auto; flex:0 0 auto; padding-bottom:2px; }} .tab {{ background:#10182c; color:var(--text); border:1px solid var(--line); white-space:nowrap; }} .tab.active {{ background:var(--accent); color:#07101f; }} .tab-panel {{ display:none; min-height:0; overflow:auto; padding-right:4px; }} .tab-panel.active {{ display:block; flex:1; }}
#cfgForm {{ min-height:0; flex:1; display:flex; flex-direction:column; overflow:hidden; }}
.grid {{ display:grid; grid-template-columns:repeat(auto-fit,minmax(300px,1fr)); gap:10px; align-items:start; }}
.setting,.check {{ background:rgba(6,10,20,.32); border:1px solid rgba(255,255,255,.06); border-radius:13px; padding:10px; min-width:0; }}
...
input:not([type="checkbox"]),select,textarea {{ width:100%; border-radius:11px; border:1px solid #314264; background:#08101f; color:var(--text); padding:9px; outline:none; }} input:not([type="checkbox"]):focus,select:focus,textarea:focus {{ border-color:var(--accent); box-shadow:0 0 0 3px rgba(124,199,255,.16); }}
...
textarea {{ min-height:90px; resize:vertical; }} pre {{ white-space:pre-wrap; overflow-wrap:anywhere; background:#07101f; border:1px solid var(--line); border-radius:12px; padding:9px; }} .ok {{ color:var(--good); }} .bad {{ color:var(--bad); }} .warn {{ color:var(--warn); }} .toast {{ min-height:18px; color:var(--warn); font-size:12px; }} .section-title {{ margin:0 0 6px; font-size:20px; }} .explain {{ color:var(--muted); margin:0 0 11px; line-height:1.4; }} .actions-row {{ display:flex; flex-wrap:wrap; gap:8px; margin:10px 0 12px; }} .actions-row button {{ white-space:normal; }}
.plan-preview {{ display:grid; gap:7px; max-height:330px; overflow:auto; padding-right:4px; }} .plan-item {{ display:grid; grid-template-columns:38px 82px minmax(0,1fr) 54px; gap:8px; align-items:start; padding:9px; border-radius:12px; border:1px solid rgba(255,255,255,.07); background:rgba(0,0,0,.16); }} .plan-item.speech {{ border-color:rgba(124,199,255,.28); }} .plan-item.music {{ border-color:rgba(98,215,141,.22); }} .plan-item .title {{ font-weight:700; overflow-wrap:anywhere; }} .plan-item .text {{ color:#cbd8f5; margin-top:4px; font-size:12px; overflow-wrap:anywhere; }}
...
<div class="layout">
  ...
  <main>
    ...
    <form id="cfgForm">
      ...
      <section class="tab-panel active" data-panel="dash"><div class="card"><h2 class="section-title">Пульт эфира</h2><p class="explain">Здесь видно, что реально происходит: готовится ли план, строятся ли описания музыки, готова ли вставка ведущих. Кнопки ниже запускают задачи без перезапуска консоли.</p><div class="actions-row"><button type="button" id="showPlanGenBtn">🧠 Сгенерировать план</button><button type="button" id="trackProfileBtn">🎧 Описания треков из интернета</button><button type="button" id="showPlanClearBtn" class="secondary">Очистить план</button><button type="submit" class="secondary">💾 Сохранить настройки</button></div><div class="progress"><b>План:</b> <span id="showPlanStatus">{esc(snap.get('show_plan_status',''))}</span><div class="bar"><div id="showPlanFill" class="fill"></div></div><div class="mini" id="showPlanDetail"></div></div><div class="progress"><b>Описания музыки:</b> <span id="trackProfileStatus">{esc(snap.get('track_profile_status',''))}</span><div class="bar"><div id="trackProfileFill" class="fill"></div></div><div class="mini" id="trackProfileDetail"></div></div><p><b>Заранее готовая речь:</b> <span id="preparedStatus">{esc(snap.get('prepared_status',''))}</span></p><p><b>Модель LM:</b> <span id="usedModel">{esc(snap.get('used_lm_model',''))}</span></p><p><b>Последняя ошибка:</b> <span id="lastError">{esc(snap.get('last_error','нет'))}</span></p><h3>Последняя фраза ведущих</h3><textarea id="hostText" readonly>{esc(snap.get('last_host_text') or 'пока нет')}</textarea></div></section>
